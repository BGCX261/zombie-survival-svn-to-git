...
public class Zombie extends Humanoid {
	...
	public void takeTurn(ArrayList<Humanoid> humanList) {
		...
		for (Humanoid human : humanList) {
			if (ZombieSurvival.getDistanceBetween(this, human) 
					< ZombieSurvival.getDistanceBetween(this,closestHuman)) {
				closestHuman = human;
			}
		}
		// if no human was close enough to see lets wander randomly
		if (ZombieSurvival.getDistanceBetween(this, closestHuman) > detectionDistance) {
			super.takeTurn();
		} else {
